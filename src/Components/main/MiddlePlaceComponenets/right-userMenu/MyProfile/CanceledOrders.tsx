...
import { FaChevronLeft, FaFileInvoice, FaTimesCircle, FaHourglassHalf, FaQuestionCircle } from "react-icons/fa"
...
// آیکون بر اساس مرحله‌ی پردازش سفارش لغوشده، به‌صورت داینامیک انتخاب می‌شود
function getProcessIcon(processStatus: Order['processStatus']) {
    switch (processStatus) {
        case 'completed':
            return <FaTimesCircle className="text-red-500" title="لغو شد" />
        case 'in-progress':
        ...
        <>
            <section className="flex flex-col h-fit w-full dark:text-white">
                <div className="flex flex-col w-full h-full justify-center items-center ">
                    <div className="border-b-2 border-blue-500 font-bold">سفارش های لغو شده</div>
                </div>
                ...
                {orders && orders.length > 0 ? (
                                ...
                                <div className="flex justify-between items-start text-sm">
                                    <div className="flex flex-col lg:flex-row gap-3">
                                        ...
                                        <span className="text-gray-200 lg:flex lg:flex-row gap-1">
                                            تاریخ لغو: <span className="text-gray-400">{formatOrderDate(order.orderDate)}</span>
                                        </span>
                                        <span className="text-gray-200 lg:flex lg:flex-row gap-1">
                    ...
                    <div className="flex flex-col w-full h-full justify-center items-center ">
                        <div className="w-1/2 h-1/2">
                            <img src="/pictures/document-list.svg" alt="سفارش لغوشده‌ای ندارید" className="w-full h-full object-contain" />
                        </div>
                        <p className="text-gray-400 text-lg">هیچ سفارش لغوشده‌ای ندارید</p>
                    </div>
                )}
            </section>
        </>
