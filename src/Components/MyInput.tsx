...
// * قبلا برای ذخیره سازی مقادیر از localStorage استفاده میشد اما الان از دیتابیس استفاده شده است و این قسمت کامنت شده است
...
function MyInput() {
    ...
    //**قبلا برای localStorage استفاده میشده 
    ...
    function MyInputuserName() {
        ...
        async function handleButton() {
            ...
            //** قبلا برای چک کردن مقادیر از local storage استفاده میشد اما الان از دیتابیس استفاده شده است و این قسمت کامنت شده است
            ...
            // بررسی تکراری نبودن نام کاربری از طریق دیتابیس و سرور
            try {
            ...
            try {
                ...
                const response = await axios.post('http://localhost:8000/api/add-users', newUser);

                // console.log(response.data);

                // بعد از ذخیره شدن اطلاعات در دیتابیس ، کاربر به صفحه ی کاربری خودش هدایت می شود
                navigate(`/TheUserPage/${result}/main`)
            } catch (error) {  
                console.log(error);
            }   

        // **قبلا برای ذخیره در localStorage بوده 
    ...
    async function handleClickuserEmail(e: FormEvent) {
        ...
        //***این قسمت قبلا برای localStorage استفاده میشده 
        ...
        //گرفتن اطلاعات از سرور بررسی تکراری نبودن ایمیل
        try {
            const response = await axios.get(
        ...
        //*** قبلاً برای چک کردن ایمیل‌های موجود از LocalStorage استفاده میشد
        // بررسی میکند آیا مقدار ایمیل وارد شده در فرم توسط کاربر ، آیا از قبل وجود داشته یا نه
        // if (isEmailExists) {
        //     setButtonStyle("dark:bg-slate-900  border-2 p-1 rounded-lg text-base w-full text-red-600 border-red-600")
        //     setInputStyle("p-1  dark:bg-slate-900 w-full rounded-lg border-2 border-red-600 focus-visible:outline-none dark:caret-teal-400")
